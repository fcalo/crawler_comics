...
class CrawlerComics_2(CrawlerComics):
    ...
    def download_url(self, url, level = False):
        ...
        post = {'contrasenya':self.config['password'], 'usuari':self.config['login']}
        
        datagen, headers = multipart_encode(post)
        ...
        req = urllib2.Request(self.config['url_login'], datagen, headers)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        crawl = CrawlerComics_2(verbose = True)
        crawl.run()
    else:
